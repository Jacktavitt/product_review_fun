...
def ParseReviews(asin):
	...
	XPATH_REVIEW_SECTION_2_ALT = '//div[@id="cm_cr-review_list"]'

	XPATH_AGGREGATE_RATING = '//table[@id="histogramTable"]//tr'
	XPATH_PRODUCT_NAME = '//h1//span[@id="productTitle"]//text()'
	XPATH_PRODUCT_NAME_2 = '//head//title//text()'
	XPATH_PRODUCT_PRICE  = '//span[@class="a-color-price arp-price"]/text()'
	XPATH_PRODUCT_PRICE_2  = '//span[@id="priceblock_ourprice"]/text()'
	
	raw_product_price = parser.xpath(XPATH_PRODUCT_PRICE)
	if not raw_product_price:
		raw_product_price = parser.xpath(XPATH_PRODUCT_PRICE_2)
	product_price = ''.join(raw_product_price).replace(',','')

	raw_product_name = parser.xpath(XPATH_PRODUCT_NAME)
	if not raw_product_name:
		raw_product_name = parser.xpath(XPATH_PRODUCT_NAME_2)
	...
	if not reviews:
		reviews = parser.xpath(XPATH_REVIEW_SECTION_2_ALT)
	ratings_dict = {}
	reviews_list = []
	...
	if reviews:
		...
		return {'reviews': None,'reason':raw_product_name}
	# 	except ValueError:
...
def ReadAsin():
	...
	for url in UrlList:
		...
		sleep(2)
	with open('data_huge_test.json','w') as f:
